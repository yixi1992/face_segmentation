...
iter = range(5, 13960,100)
model_acc = np.zeros(len(iter))
for idx,i in enumerate(iter):
	model_file = 'snapshots/snapshot_face_segmentation_finetune_lr1e-6__iter_'+str(i)+'.caffemodel.h5'
	if not os.path.exists('pred_visual/'+model_file[len('snapshots/snapshot_face_segmentation_finetune_'):len('snapshots/snapshot_face_segmentation_finetune_lr1e-6__iter_'+str(i))] + '/'):
		os.makedirs('pred_visual/'+model_file[len('snapshots/snapshot_face_segmentation_finetune_'):len('snapshots/snapshot_face_segmentation_finetune_lr1e-6__iter_'+str(i))] + '/')
	...
	plt.savefig('test_accuracy_lr1e-6.png')
