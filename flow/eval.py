...
if True:
	model = 'snapshots_camvid300flow_train_lr1e-10'
	lmdb_dir = 'camvid300flow_lmdb'
	work_dir = '/lustre/yixi/face_segmentation_finetune/flow/model1'
	deploy_file = os.path.join(work_dir, 'deploy.prototxt')
	snapshot = os.path.join(work_dir, 'snapshots_camvid300flow/train_lr1e-10/_iter_{snapshot_id}.caffemodel')
	pred_visual_dir_template = os.path.join(work_dir, 'pred_visual_camvid300flow/train_lr1e-10/_iter_{snapshot_id}')
	...
	input_RGB_mean = {'Train':(78.39410533, 76.82482302, 74.28814409, 92.92632524, 97.68362453),
			'Test':(78.39410533, 76.82482302, 74.28814409, 92.92632524, 97.68362453)}
